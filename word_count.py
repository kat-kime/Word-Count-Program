    ...
    with open(source) as f:
        for line in f:
            ...
            for w in words:
                ...
                # find out if table contains key
                if ht.get(word):
                    # if it does, get the old value
                    old_value = ht.get(word)
                ...
                # put key in set of keys
                keys.add(word)

                # if table load is >= 8, resize table with double capacity
                if ht.table_load() >= 8:
                    ht.resize_table(ht.capacity * 2)

    # get top words
    # iterate through keys
    for key in keys:
        # add (value,key) pair to totals
        totals.append((ht.get(key), key))

    # sort totals list in descending order
    totals = sorted(totals, reverse=True)

    # replace values with correct arrangement
